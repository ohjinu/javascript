// 배열.find(함수식)  // 콜백함수 조건에 맞는 첫 요소
// 배열.findIndex(테스트함수) // 콜백함수 조건에 맞는 첫 요소인덱스

const myArray = ['홍길동', '홍길순', '길순이', '이순신', '강감찬']

const targetUser = myArray.find((data) => data == '길순이') // T or F를 판별해 data==길순이 값을 targetUser에 반환
console.log(targetUser)

const targetUserIndex = myArray.findIndex((data) => data == '길순이') // T or F를 판별해 data==길순이 값을 targetUser에 반환
...
function findUser(searchId) {
    ...
    if (targetData == null) { // 해당 데이터가 없으면
        ...
        // searchResult.textContent = '<h1>유저 검색 결과 없음</h1>'
        return
    }
    //검색결과의 이름 표시
