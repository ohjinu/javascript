...
// 버튼 요소에 이벤트등록
document.querySelectorAll('.button').forEach((btn) => {  // 버튼 묶어!
    // console.log(btn)
    btn.addEventListener('click', (e) => {  // 어떤 요소 클릭했는지

        // console.log(e)  // 클릭한 버튼의 객체정보
        // alert('버튼 클릭:'+ e.target.dataset.age)
        
        // 클릭한 버튼의 요소
        const button = e.target
        // 버튼의 요소에서 data속성 가져오기
...
console.log("-------- 유사 배열 -> 배열 -> map() -> reduce() -> 최종 배열 -------")
const str1 = [...myString]
              .map((ch)=> `${ch}!!`)
              .reduce((a,b)=> a+b)
